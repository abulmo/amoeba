...
class Amoeba {
	...
	static void printWeights(const ref double [] weight, std.stdio.File f = stdout) {
		...
		f.write("\t"); foreach(i; 35 ..  43) f.writef("%+7.4f, ", weight[i]); f.writeln("// positional  [35-48]");
		f.write("\t"); foreach(i; 43 ..  49) f.writef("%+7.4f, ", weight[i]); f.writeln("");
		f.write("\t"); foreach(i; 49 ..  53) f.writef("%+7.4f, ", weight[i]); f.writeln("                                    // P structure [49-56]");
		f.write("\t"); foreach(i; 53 ..  57) f.writef("%+7.4f, ", weight[i]); f.writeln("");
		f.write("\t"); foreach(i; 57 ..  58) f.writef("%+7.4f, ", weight[i]); f.writeln("                                                               // tempo [57]");
		f.writeln("\t// Endgame");
		f.write("\t"); foreach(i; 58 ..  65) f.writef("%+7.4f, ", weight[i]); f.writeln(" // material    [58-64]");
		f.write("\t"); foreach(i; 65 ..  71) f.writef("%+7.4f, ", weight[i]); f.writeln("          // mobility    [65-70]");
		f.write("\t"); foreach(i; 71 ..  77) f.writef("%+7.4f, ", weight[i]); f.writeln("          // attack      [71-76]");
		f.write("\t"); foreach(i; 77 ..  83) f.writef("%+7.4f, ", weight[i]); f.writeln("          // defense     [77-82]");
		f.write("\t"); foreach(i; 83 ..  88) f.writef("%+7.4f, ", weight[i]); f.writeln("                   // K attack    [83-87]");
		f.write("\t"); foreach(i; 88 ..  93) f.writef("%+7.4f, ", weight[i]); f.writeln("                   // K defense   [88-92]");
		f.write("\t"); foreach(i; 93 ..  98) f.writef("%+7.4f, ", weight[i]); f.writeln("                   // positional  [93-97]");
		f.write("\t"); foreach(i; 98 .. 102) f.writef("%+7.4f, ", weight[i]); f.writeln("                            // P structure [98-105]");
		f.write("\t"); foreach(i;102 .. 106) f.writef("%+7.4f, ", weight[i]); f.writeln("");
		f.write("\t"); foreach(i;106 .. 107) f.writef("%+7.4f, ", weight[i]); f.writeln("                                                       // tempo [106]");
		f.writeln("];"); 
		f.flush();
	}
